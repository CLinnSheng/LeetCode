 ...
 * everytime we insert a new eleemtn Intuition: Sort the element everytime we
 * want to find the media. And then get the middle element. Time Complexity:
...
public:
  ...
  double findMedian() {
    return (right_half.size() + left_half.size()) % 2
               ? (right_half.size() > left_half.size() ? right_half.top()
                                                       : left_half.top())
